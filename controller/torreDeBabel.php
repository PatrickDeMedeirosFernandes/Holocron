...
//camada 1
        ...
        if (buscaPorPersonagem($text, 0) != ' ') {
            $saida = buscaPorPersonagem($text);
        }
...
//            $respostaATT = "NÃO SEI O Q FAZER, NÃO TENHO RESPOSTA PRA ISSO";
            $saida = '';
            if (buscaPorPersonagem($kayword, 0) != ' ') {
                $saida ='Então a resposta para sua pergunta é: '. buscaPorPersonagem($kayword);
          ///      echo $saida;
            } else {
                 $saida = BuscaDefaut($kayword, true);
              //  $saida = 'Buscafautresp, true)';
            }
            $result = $saida;
             $respostaATT = saldacao($result);
        } else {
            $bi = $Usuario;
